...
- **完整的新手引导** —— 首次启动跟着遮罩指引走一遍，随时可在设置里重看。

## 快速上手

1. 首次打开会进入「新手引导」，跟着提示点一遍就熟悉了。
2. 在**日历**上点某一天 → 选分类、输入金额 →「完成」。
3. 想自动记账：设置 → 功能 →「固定收支」；想被提醒：设置 → 功能 →「记账提醒」。
4. 想备份 / 换机：设置 → 功能 →「云端备份」或「导入 / 导出」。
5. 换主题、关启动动画：设置 → 外观。

> 设置入口：在**日历**页再点一次底部「日历」页签。
