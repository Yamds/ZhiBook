...
import { RADIUS_LABEL_KEYS } from '../../core/design/radius';
...
interface ThemeItem { value: ThemeMode; labelKey: string; canvas: string; sidebar: string; text: string; subtext: string; brand: string; accent: string; }
interface ThemeGroup { labelKey: string; items: ReadonlyArray<ThemeItem>; }

/**
 * 主题预览表直接由注册表（`core/design/themes.ts`）推导：
 * 新主题只需在注册表里加一行 + 在 `tokens.css` 里加一个块 + 在语言文件里补两条 key，
 * 设置页自动出现。
 */
...
export function StartupTabSegment({ value, onChange }: { value: StartupTab; onChange: (next: StartupTab) => void }) {
    const { t } = useTranslation();
    const items = STARTUP_TABS.map((tab) => ({ value: tab.value, labelKey: tab.labelKey }));
    return (
