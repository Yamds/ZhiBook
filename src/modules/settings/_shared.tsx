...
export function MotionLevelSegment({ value, onChange, disabled }: { value: MotionLevel; onChange: (next: MotionLevel) => void; disabled?: boolean }) {
    const items: ReadonlyArray<{ value: MotionLevel; label: string; icon: ComponentType<LucideProps> }> = [{ value: 'elegant', label: '优雅', icon: Feather }, { value: 'standard', label: '标准', icon: Wand2 }, { value: 'rich', label: '丰富', icon: Sparkles }];
...
export function RadiusStyleSegment({ value, onChange }: { value: RadiusStyle; onChange: (next: RadiusStyle) => void }) {
    const items: ReadonlyArray<{ value: RadiusStyle; label: string; icon: ComponentType<LucideProps> }> = [{ value: 'square', label: RADIUS_LABELS.square, icon: Square }, { value: 'standard', label: RADIUS_LABELS.standard, icon: RectangleHorizontal }, { value: 'round', label: RADIUS_LABELS.round, icon: Circle }];
